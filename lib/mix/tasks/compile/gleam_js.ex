defmodule Mix.Tasks.Compile.GleamJs do
    ...
    case OptionParser.parse(args, switches: @switches) do
      {options, _, _} ->
        ...
        if gleam? do
          ...
          compiled_files = compile(app)

          bundle(compiled_files, config, app)
        end
    end
  ...
  defp compile(app) do
    ...
    erl_build_lib = Path.join([Mix.Project.build_path(), "lib"])
    erl_build_app = Path.join([erl_build_lib, "#{app}"])
    ...
    package =
      cond do
        ...
        File.regular?(Path.join(erl_build_app, "gleam.toml")) ->
          erl_build_app
        ...
        true ->
          ...
          |> Enum.each(fn dir ->
            ...
            dest = Path.join(erl_build_app, dir)
            File.rm_rf!(dest)
  ...
  defp bundle(compiled_files, config, app) do
